...
static int simple_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi) {
	struct directory_entry *p;
...
static int simple_unlink(const char *path) {
	...
	// Search a file
	for (q = root, p = root; p != NULL; q = p, p = p->next) {
...
static int simple_rename(const char *from, const char *to) {
	struct directory_entry *p;

	// No such file or directory
	if ((p = search_file(from)) == NULL) {
		return -ENOENT;
	}

	// If destination file exists, remove it
	if (search_file(to) != NULL) {
		simple_unlink(to);
	}
...
static int simple_truncate(const char* path, off_t size) {
	...
	// Check existence
	if ((p = search_file(path)) == NULL) {
		return -ENOENT;
	}

	// Clear
	memset(p->file->data, 0, p->file->length);

	// Need more memory
	if (size > p->file->capacity) {
